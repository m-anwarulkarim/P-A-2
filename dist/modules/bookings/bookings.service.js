...
const createBooking = async (payload) => {
    ...
    `, [vehicle_id]);
        ...
        const totalTDate = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
        // -------- check bookings
...
// Get All Bookings -------------------------------
const getAllBookings = async (users) => {
...
// ========================================================================
const updateBookings = async (id, payload, user) => {
    ...
    if (status === "cancelled") {
        ...
        const updated = await pool.query(`UPDATE bookings SET status='cancelled' WHERE id=$1 RETURNING *`, [id]);
    ...
    if (status === "returned") {
        ...
        const updated = await pool.query(`UPDATE bookings SET status='returned' WHERE id=$1 RETURNING *`, [id]);
        await pool.query(`UPDATE vehicles SET availability_status='available' WHERE id=$1`, [booking.vehicle_id]);
        return updated.rows[0];
    }
...
const autoReturnBookings = async () => {
    ...
    for (const bk of expired.rows) {
        await pool.query(`UPDATE vehicles SET availability_status='available' WHERE id=$1`, [bk.vehicle_id]);
    }
    return expired.rows;
};
